...
class LogisticLogisticRegressionSelectorSpec( unittest.TestCase ):
    def setUp( self ):
        self.__PM = PropertiesManager()

    def test_it_is_a_Selector( self ):
        MySelect = LogisticRegressionSelector( self.__PM )
        self.assertTrue( isinstance( MySelect, Selector ) )

    @patch( 'biomed.vectorizer.selector.logistic_regression_selector.SelectFromModel' )
    @patch( 'biomed.vectorizer.selector.logistic_regression_selector.LogisticRegression' )
